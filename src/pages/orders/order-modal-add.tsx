...
export default function OrderModalAdd({
  ...
  const [forecasts, setForecast] = useState<{ number_forecast: string }[]>([]);
  ...
  const handleSubmit = async (values: OrderFormValues) => {
    try {
      ...
      await apiBaseClient.post(`/api/orders/create-edi-order`, formData, {
        headers: {
      ...
      console.error(err);
      toast.error((error as Error).message || "Create Order failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVendorChange = (value: string) => {
    form.resetFields(["number_forecast"]);
    setForecast([]);
    form.setFieldsValue({ vendor_code: value });
    fetchForecastList(value);
  };

  const fetchForecastList = async (vendorCode: string) => {
    try {
      const res = await apiBaseClient.get(
        `/api/orders/get-order-by-vendor-code?vendor_code=${encodeURIComponent(vendorCode)}`
      );
      const data = res.data;
  ...
  return (
    <Modal
      ...
      footer={null}
      title={<span className="text-lg font-semibold">Add Order</span>}
      destroyOnClose
      width={750}
      centered
    >
      <Form
        ...
        className="mt-6"
      >
        {/* Row 1: Order No. & Vendor Code */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Form.Item
            label={<span className="font-medium text-gray-700">Order No.</span>}
            name="number_order"
            rules={[{ message: "Please input order number" }]}
          >
            <Input
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Form.Item
            ...
            rules={[{ required: true, message: "Please select number forecast" }]}
          >
            <Select
              placeholder="Select vendor"
              size="large"
              ...
              showSearch
              optionFilterProp="label"
              options={forecasts.map(v => ({
                value: v.number_forecast,
                label: `${v.number_forecast}`,
              }))}
            />
          </Form.Item>
        </div>
