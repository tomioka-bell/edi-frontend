...
import ForecastModalAdd from "./forecast-modal-add";
...
export default function ForecastPage() {
  ...
  const fetchForecasts = React.useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiBaseClient.get("/api/forecasts/get-forecast-active-top", {
        params: { limit: 10, vendor_code: user?.group },
      });
      const data: ForecastRow[] = res.data;
      const mapped: ForecastRow[] = (data ?? []).map((d) => ({
        id: d.edi_forecast_id,
        edi_forecast_id: d.edi_forecast_id,
        number_forecast: d.number_forecast,
        vendor_code: d.vendor_code,
        status_forecast: d.status_forecast ?? null,
        ...
        read_forecast: d.read_forecast ?? null,
  ...
  const filteredRows = React.useMemo(() => {
    ...
    if (statusFilter) {
      filtered = filtered.filter(row => row.status_forecast === statusFilter);
    }
  ...
  const handleOpenForecast = React.useCallback(
    async (row: ForecastRow) => {
      const forecastNo = row.number_forecast;
      const forecastId = row.edi_forecast_id;

      if (!row.read_forecast && user?.source_system !== "APP_EMPLOYEE") {
        try {
      ...
      navigate(withLang(lang, `/forecast-form/${forecastNo}`));
    },
    [lang, navigate, user]
  );

  const handlePreviewFile = (row: ForecastRow) => {
  ...
  const handleDownloadFile = (row: ForecastRow) => {
    if (!row.file_url) return;
    const url = buildImageURL(row.file_url);

    const filename = `Forecast-${row.number_forecast}${url.toLowerCase().endsWith(".pdf") ? ".pdf" : ""
      }`;
    ...
    return [
      {
        ...
        Cell: ({ row }) => (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              void handleOpenForecast(row.original);
            }}
          >
            {row.original.number_forecast}
          </button>
        ),
      },
  ...
  return (
    <Layout>
      <div className="flex items-center gap-2 pb-4">
        ...
        <h1 className="text-2xl ml-4 font-semibold text-root">Forecast</h1>
      </div>
      ...
      <div className="card-root px-4 py-4 rounded-lg shadow-lg">
        ...
        <ForecastModalAdd isOpen={isModalAddOpen} onClose={() => setIsModalAddOpen(false)} onCreated={fetchForecasts} />

        <div>
          <hr className="w-48 h-1 mx-auto my-6 bg-gray-200 border-0 rounded-sm"></hr>
        </div>

        <div className="card-root rounded-lg shadow-sm p-2 bg-linear-to-r from-sky-50 to-blue-100">
          <div className="flex justify-between items-center pb-4 gap-4 flex-wrap">
            ...
            {user?.source_system === "APP_EMPLOYEE" && (
              ...
              >
                <GrAdd style={{ color: "white" }} className="w-4 h-4 hover:animate-spin text-root" />
                Add Forecast
              </button>
            )}
          </div>

          <MaterialReactTable<ForecastRow & MRT_RowData>
            columns={columns}
            data={filteredRows}
            state={{
              isLoading: loading,
