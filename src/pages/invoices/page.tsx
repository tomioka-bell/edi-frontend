...
export default function InvoicesPage() {
  ...
  const fetchInvoice = React.useCallback(async () => {
    try {
      setLoading(true);
      const res = await apiBaseClient.get(`/api/invoice/get-invoices-active-top?limit=10&vendor_code=${user?.group}`);
      const data: InvoiceRow[] = res.data;
      const mapped: InvoiceRow[] = (data ?? []).map((d) => ({
        id: d.edi_invoice_id,
        edi_invoice_id: d.edi_invoice_id,
        number_invoice: d.number_invoice,
        vendor_code: d.vendor_code,
        status_invoice: d.status_invoice ?? null,
        ...
        read_invoice: d.read_invoice ?? null,
  ...
  const filteredRows = React.useMemo(() => {
    ...
    // Apply status filter
    if (statusFilter) {
      filtered = filtered.filter(row => row.status_invoice === statusFilter);
    }
  ...
  const handleOpenInvoice = React.useCallback(
    async (row: InvoiceRow) => {
      const InvoiceNo = row.number_invoice;
      const invoiceId = row.edi_invoice_id;

      if (!row.read_invoice && user?.source_system !== "APP_USER") {
        try {
          const res = await apiBaseClient.patch(
            `/api/invoice/mark-invoice-as-read/${invoiceId}`
          );
          ...
          if (res.status !== 200 && res.status !== 204) {
            ...
            setRows((prev) =>
              prev.map((r) =>
                r.edi_invoice_id === invoiceId
                  ? { ...r, read_forecast: true }
                  : r
              )
            );
          }
      ...
      navigate(withLang(lang, `/invoice-form/${InvoiceNo}`));
    },
    [lang, navigate, user]
  );
  ...
  const handlePreviewFile = (row: InvoiceRow) => {
  ...
  const handleDownloadFile = (row: InvoiceRow) => {
    if (!row.file_url) return;
    const url = buildImageURL(row.file_url);

    const filename = `Invoice-${row.number_invoice}${url.toLowerCase().endsWith(".pdf") ? ".pdf" : ""}`;

    downloadFile(url, filename);
  };

  const columns = React.useMemo<MRT_ColumnDef<InvoiceRow>[]>(() => {


    return [
      {
        header: "Invoice No.",
        accessorKey: "number_invoice",
        size: 160,
        Cell: ({ row }) => (
          <button
            type="button"
            className="text-[#08a4b8] hover:underline bg-transparent bInvoice-0 p-0 cursor-pointer"
            onClick={(e) => {
              e.stopPropagation();
              void handleOpenInvoice(row.original);
            }}
          >
            {row.original.number_invoice}
          </button>
        ),
  ...
  return (
    <Layout>
      <div className="flex items-center gap-2 pb-4">
        ...
        <h1 className="text-2xl ml-4 font-semibold text-root">Invoice</h1>
      </div>

      <InvoiceModalAdd isOpen={isModalAddOpen} onClose={() => setIsModalAddOpen(false)} onCreated={fetchInvoice} />
      <div className="card-root px-4 py-4 rounded-lg shadow-lg">
        ...
        <div className="bg-linear-to-r from-sky-50 to-blue-100 rounded-lg shadow-sm p-2">
          ...
          <MaterialReactTable<InvoiceRow & MRT_RowData>
            columns={columns}
            data={filteredRows}
            state={{ isLoading: loading, showGlobalFilter: true, globalFilter }}
          ...
          {previewUrl && (
            <>
              {previewUrl.toLowerCase().endsWith(".pdf") ? (
                <iframe
                  src={previewUrl}
                  className="w-full h-[70vh] bInvoice-0"
                  title="PDF Preview"
                />
              ) : (
                <img
