...
import type { InvoiceDetailResp } from "../../types/invoices-form";
...
import InvoicesStatusLogs from "./invoice-status-logs";
...
export default function InvoicesFormPage() {
    const { number } = useParams<{ number: string }>();
    const [data, setData] = React.useState<InvoiceDetailResp | null>(null);
    ...
    const reloadInvoice = React.useCallback(async () => {
        try {
            ...
            const res = await apiBaseClient.get(`/api/invoice/get-invoice-detail-by-number?number_invoice=${number}`);

            const json: InvoiceDetailResp = res.data;
            ...
            setError((e as Error)?.message ?? "เกิดข้อผิดพลาดในการโหลดข้อมูล");
        } finally {
            setLoading(false);
        }
    }, [number]);
    ...
    React.useEffect(() => {
        if (!data) return;
        if (user?.source_system === "APP_USER") return;

        const invoiceId = data.edi_invoice_id;
        const alreadyRead = data.read_invoice;
        ...
        const markAsRead = async () => {
            ...
            try {
                await apiBaseClient.patch(`/api/invoice/mark-invoice-as-read/${invoiceId}`);
                setData(prev => prev ? { ...prev, read_invoice: true } : prev);
    ...
    return (
        <Layout>
            <InvoiceConfirmModal edi_invoice_id={data?.edi_invoice_id ?? ""} edi_invoice_version_id={selectedVersion?.edi_invoice_version_id ?? ""}
                status_invoice={data?.status_invoice ?? ""} visible={isModalVisible} onOk={handleOk} onCancel={handleCancel} createdbyexternalID={user?.external_id} source_system={user?.source_system} />
            <InvoicesVersionAdd ediInvoiceId={data?.edi_invoice_id ?? ""} visible={isVersionModalVisible} onOk={handleOk} onCancel={handleVersionCancel} />
            <div className="mb-6 flex items-center justify-between">
                <div className="flex items-start gap-4 pb-2">
                    <div className="relative w-12 h-12 bg-gray-50 rounded-2xl flex items-center justify-center shadow-lg">
                        <LiaFileInvoiceDollarSolid className="w-5 h-5 text-gray-600" />
                    </div>
            ...
            <div className="flex justify-between mb-2">
                <Link
                    to="/en/Invoices"
                    className="inline-flex items-center gap-2 rounded-lg broder broder-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition-all duration-200 hover:broder-slate-300 hover:bg-slate-50"
                >
            ...
            {loading ? (
                ...
                <div className="rounded-2xl broder broder-rose-200 bg-rose-50 p-6 text-rose-700">
                    Error: {error}
                </div>
            ) : !data ? (
                <div className="rounded-2xl broder broder-slate-200 bg-white p-6 text-slate-600">
                    No data found
                </div>
            ) : (
                <div className="space-y-6">
                    {/* Header Card */}
                    <div className="rounded-2xl broder broder-slate-200 bg-white p-6 shadow-sm">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-cyan-100 text-cyan-700 shadow-inner">
                                    <span className="text-xl font-bold">
                                        {data.number_invoice?.slice(0, 2) || "OR"}
                                    </span>
                                </div>
                                <div>
                        ...
                        <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            ...
                            <div className="rounded-xl border border-slate-100 p-4">
                                <div className="text-xs text-slate-500">Number Order</div>
                        ...
                        <div className="pt-10">
                            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="text-sm text-slate-600">Select Version :</span>
                                    <Select
                                        size="middle"
                                        className="min-w-36"
                            ...
                            {(!data.versions || data.versions.length === 0) && (
                                <div className="rounded-xl broder broder-slate-100 bg-slate-50 p-6 text-center text-slate-600">
                                    ยังไม่มีเวอร์ชัน
                                </div>
                            )}
                            {selectedVersion && (
                                <div
                                    className={[
                                        "rounded-xl broder p-6 transition-all duration-200",
                                        selectedVersion.is_active
                                            ? "broder-cyan-300 bg-cyan-50/50 ring-2 ring-cyan-200 shadow-sm"
                                            : "broder-slate-200 bg-white shadow-sm",
                                    ].join(" ")}
                                >
                                    {/* Header Section */}
                                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-4 broder-b broder-slate-200">
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            <span
                                                className={[
                                                    ...
                                                    statusColor(selectedVersion.status_invoice),
                                                ].join(" ")}
                                            >
                                                {selectedVersion.status_invoice || "-"}
                                            </span>
                                    ...
                                    <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-4">
                                        {/* Period From */}
                                        <div className="rounded-lg broder broder-slate-200 bg-slate-50 p-4 transition-all hover:broder-slate-300 hover:shadow-sm">
                                        ...
                                        {/* Note */}
                                        <div className="rounded-lg broder broder-slate-200 bg-slate-50 p-4 transition-all hover:broder-slate-300 hover:shadow-sm lg:col-span-1 md:col-span-2">
                                            <div className="flex items-center gap-2 text-xs font-medium text-slate-500 uppercase tracking-wide mb-2">
                                                <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path
                                                        strokeLinecap="round"
                                    ...
                                    {/* Action Buttons */}
                                    <div>
                                        <ViewFile file_url={selectedVersion.source_file_url} number_invoice={data?.number_invoice} />
                                    </div>
                                </div>
                            )}

                            <div className="py-8">
                                <InvoicesTable versionId={data?.edi_invoice_id || ""} number={data.number_invoice} />
                            </div>
                            ...
                            <div className="mt-8 flex items-center gap-4 p-4 rounded-2xl broder broder-slate-200 bg-white shadow-sm hover:shadow-md transition-all duration-300">
                                {/* Avatar ผู้สร้าง */}
                                <Avatar
                                ...
                                <div className="flex flex-col justify-center">
                                    ...
                                    {/* Note */}
                                    <div className="mt-2 text-sm text-slate-700 bg-slate-50 rounded-xl px-3 py-2 broder broder-slate-100 shadow-inner max-w-fit">
                            ...
                            {/* Status Logs */}
                            <div className="mt-2">
                                <InvoicesStatusLogs invoiceVersionId={data?.edi_invoice_id ?? ""} />
                            </div>
