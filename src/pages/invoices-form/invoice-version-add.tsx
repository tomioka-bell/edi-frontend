...
import type { InvoiceVersionFormValues } from "../../types/invoices-add-form";
...
export default function InvoicesVersionAdd({
    visible,
    onOk,
    onCancel,
    ediInvoiceId,
    ...
    const [values, setValues] = useState<InvoiceVersionFormValues>({
        edi_invoice_id: ediInvoiceId,
        period_from: null,
        period_to: null,
        status_invoice: "New",
        read_invoice: false,
    ...
    const submitVersion = async (payload: InvoiceVersionFormValues, file: File | null) => {
        ...
        const res = await apiBaseClient.post(`/api/invoice/create-invoice-version`, formData, {
            headers: {
    ...
    const handleSubmit = async () => {
        if (!values.note) {
           toast.error("กรุณากรอก Note ให้ครบ");
            return;
        }
