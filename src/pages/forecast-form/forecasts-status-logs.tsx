...
export default function ForecastsStatusLogs({
    forecastVersionId,
}: ForecastsStatusLogsProps) {
    const [data, setData] = useState<ForecastStatusLog[]>([]);
        ...
        async function fetchStatusLog() {
            try {
                ...
                const res = await apiBaseClient.get(
                    `/api/forecasts/get-status-log-by-version-id/${forecastVersionId}`
                );

                const json = res.data;

                const logs: ForecastStatusLog[] = Array.isArray(json)
                    ? json
            ...
            {data.map((log) => {
                ...
                return (
                    <div>
                        ...
                        <div
                            key={`${log.edi_forecast_version_id}-${log.created_at}-${log.new_status}`}
