...
import type { ForecastDetailResp } from "../../types/forecast-form";
...
import ForecastsStatusLogs from "./forecasts-status-logs";
import ForecastConfirmModal from "./forecast-confirm-modal";
...
export default function ForecastFormPage() {
    const { number } = useParams<{ number: string }>();
    const [data, setData] = React.useState<ForecastDetailResp | null>(null);
    ...
    const reloadForecast = React.useCallback(async () => {
        try {
            setLoading(true);
            setError(null);

            const res = await apiBaseClient.get(
                `/api/forecasts/get-forecast-active-by-number?number_forecast=${number}`
            );
            const json: ForecastDetailResp = res.data;
            ...
            const activeVersion = json.versions?.find(v => v.is_active);
            if (activeVersion) {
                setSelectedVersionNo(activeVersion.version_no!);
            } else {
    ...
    React.useEffect(() => {
        ...
        const forecastId = data.edi_forecast_id;
        const alreadyRead = data.read_forecast;
        ...
        const markAsRead = async () => {
            ...
            try {
                await apiBaseClient.patch(
                    `/api/forecasts/mark-forecast-as-read/${forecastId}`
                );

                setData(prev => prev ? { ...prev, read_forecast: true } : prev);

            } catch (err) {
                console.error("Error mark-forecast-as-read", err);
            }
        };
    ...
    return (
        <Layout>
            <ForecastConfirmModal edi_forecast_id={data?.edi_forecast_id ?? ""} edi_forecast_version_id={selectedVersion?.edi_forecast_version_id ?? ""} status_forecast={data?.status_forecast ?? ""}
                visible={isModalVisible} onOk={handleOk} onCancel={handleCancel} created_by_external_id={user?.external_id ?? ""} source_system={user?.source_system ?? ""} />
            <ForecastVersionAdd ediForecastId={data?.edi_forecast_id ?? ""} visible={isVersionModalVisible} onOk={handleOk} onCancel={handleVersionCancel} />
            <div className="mb-6 flex items-center justify-between">
                <div className="flex items-start gap-4 pb-2">
                    <div className="relative bg-white w-12 h-12 card-bg rounded-2xl flex items-center justify-center shadow-lg">
                        <LineChartOutlined />
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="rounded-2xl border card-root p-6 shadow-sm">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-cyan-100 text-cyan-700 shadow-inner">
                                    <span className="text-xl font-bold">
                                        {data.number_forecast?.slice(0, 2) || "FC"}
                                    </span>
                                </div>
                                <div>
                                    <div className="text-sm text-slate-500">Forecast No.</div>
                        ...
                        <div className="pt-10">
                            ...
                            {selectedVersion && (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-slate-200">
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            <span
                                                className={[
                                                    ...
                                                    statusColor(selectedVersion.status_forecast),
                                                ].join(" ")}
                                            >
                                                {selectedVersion.status_forecast || "-"}
                                            </span>
                                        ...
                                        <div className="flex items-center gap-2 text-xs text-root">
                                    ...
                                    {/* Action Buttons */}
                                    <div>
                                        <ViewFile file_url={selectedVersion.source_file_url} number_forecast={data?.number_forecast} />
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="py-8">
                                <ForecastTable versionId={data?.edi_forecast_id || ""} number={data.number_forecast} />
                            </div>
                
                                <div className="py-8">
                                    <OrdersTable numberForecast={data?.number_forecast} vendorCode={data?.vendor_code}/>
                                </div>
                            ...
                            {/* Status Logs */}
                            <div className="mt-2">
                                <ForecastsStatusLogs forecastVersionId={data?.edi_forecast_id ?? ""} />
                            </div>
