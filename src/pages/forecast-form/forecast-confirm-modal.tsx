...
interface ForecastsStatusLogsProps {
...
export default function ForecastConfirmModal({
    edi_forecast_version_id,
    edi_forecast_id,
    visible,
    onOk,
    onCancel,
    ...
    const handleOk = async () => {
        try {
            ...
            formData.append("edi_forecast_version_id", edi_forecast_version_id);
            formData.append("edi_forecast_id", edi_forecast_id);
            formData.append("old_status", status_forecast ?? "");
            ...
            formData.append("created_by_source_system", source_system ?? user?.source_system ?? "");
            ...
            await apiBaseClient.post(
                `/api/forecasts/create-forecast-version-status-log`,
                formData,
                {
                    headers: {
            ...
            toast.success(
                `Confirm Forecast successfully (${statusLabelMap[selectedStatus]})`
            );
            ...
            console.error(err);
            toast.error("Confirm Forecast failed");
            setConfirmLoading(false);
        }
    };
    ...
    return (
        ...
        >
            <p className="mb-3">Are you sure you want to confirm this forecast?</p>
