...
import type {
    OrderstatusLog,
} from "../../types/orders-status";
// import ForecastTable from "./forecast-table";

interface OrdersStatusLogsProps {
    orderVersionId: string;
}

export default function OrdersStatusLogs({
    orderVersionId,
}: OrdersStatusLogsProps) {
    const [data, setData] = useState<OrderstatusLog[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        if (!orderVersionId) return;
        ...
        async function fetchStatusLog() {
            try {
                ...
                const res = await apiBaseClient.get(
                    `/api/orders/get-status-log-by-version-id/${orderVersionId}`
                );

                const json = res.data;

                const logs: OrderstatusLog[] = Array.isArray(json)
                    ? json
                ...
                setError(error.message ?? "เกิดข้อผิดพลาดในการโหลดข้อมูลสถานะ");
            } finally {
                setLoading(false);
            }
        }
    ...
    if (!orderVersionId) {
        return (
            <div className="text-sm text-gray-500">No Forecast Version found</div>
        );
    }

    if (loading) {
        return (
            <div className="text-sm text-gray-500">กำลังโหลดข้อมูลสถานะ...</div>
        );
    }
            ...
            {data.map((log) => {
                ...
                return (
                    <div>
                        ...
                        <div
                            key={`${log.edi_order_version_id}-${log.created_at}-${log.new_status}`}
